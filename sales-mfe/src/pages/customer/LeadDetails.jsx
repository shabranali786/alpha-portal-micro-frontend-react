...
const LeadDetails = () => {
  ...
  const {
    ...
    fetchData,
  } = usePaginatedData(ApiRequest.leads.leadNotes(id));
  ...
  const fetchInvoices = useCallback(
    async (filter = "all") => {
      ...
      try {
        ...
        const response = await apiAxios.get(ApiRequest.leads.invoices(id), {
          params,
        });
  ...
  useEffect(() => {
    fetchLeadDetails();
    fetchSupportingData();
  }, [fetchLeadDetails, fetchSupportingData]);

  useEffect(() => {
    if (activeTab === "invoices") {
      fetchInvoices(invoiceFilter);
    }
  }, [activeTab, invoiceFilter, fetchInvoices]);
    ...
    <div className="space-y-6 text-gray-800 transition-colors duration-300 dark:text-slate-200">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Customer #{lead.id}</h1>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <div>
            ...
            <Select
              ...
              onChange={(option) => handleStatusUpdate(option?.value || "new")}
              styles={selectStyles}
          ...
          <div>
            ...
            <Select
              ...
              value={
                assignedUser
                  ? userOptions.find((opt) => opt.value === assignedUser.id)
                  : null
              }
              onChange={(option) =>
                option ? handleAssignUser(option.value) : null
              }
              styles={selectStyles}
              isDisabled={submitting}
              isClearable
            ...
            />
            {assignedUser && (
              <p className="mt-1 text-sm text-green-600 dark:text-emerald-300">
          ...
          <div>
            ...
            <Select
              ...
              value={leadPriority}
              onChange={(option) => setLeadPriority(option)}
              styles={selectStyles}
              isDisabled={submitting}
              isClearable
                    ...
                    {invoices.map((invoice) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {getInvoiceStatusBadge(invoice.transaction_status)}
                            {invoice.paid_at && (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-400 dark:text-slate-500 text-sm mt-2">
                  {invoiceFilter === "all"
                    ...
                    : `No ${invoiceFilter} invoices available.`}
                </p>
              </div>
            )}
          </div>
