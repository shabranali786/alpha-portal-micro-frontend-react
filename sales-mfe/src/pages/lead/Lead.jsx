...
const Leads = () => {
  ...
  const canDeleteLead = usePermission(["lead.delete"]);
  const canViewLead = usePermission(["lead.show"]);
  const canViewTestLeads = usePermission(["lead.test.index"]);
  const canMarkLead = usePermission(["lead.mark-as-test"]);
  ...
  } = usePaginatedData(ApiRequest.leads.list);
  ...
  } = usePaginatedData(canViewTestLeads ? ApiRequest.leads.testList : null);
  ...
  useEffect(() => {
    if (!canViewTestLeads && activeTab === "test") {
      setActiveTab("live");
    }
  }, [canViewTestLeads, activeTab]);
  ...
  }, [buildQueryFromFilters, canViewTestLeads]);
  ...
  const handleGlobalSearch = useCallback(
    ...
    [handleLiveSearch, handleTestSearch]
  );
  ...
  }, [handleGlobalSearch, canViewTestLeads]);
  ...
  const confirmToggleTestStatus = async () => {
    ...
    try {
      ...
      toast.success(
        makeTest ? "Lead moved to Test Leads" : "Lead restored to Live Leads"
      );
      ...
      toast.error(
        error.response?.data?.message ||
          (makeTest ? "Failed to mark as test" : "Failed to restore lead")
      );
    } finally {
      setSubmitting(false);
    }
        ...
        return (
          <div className="space-y-1 relative group">
            ...
            {displayNotes.length > 0 && (
              <div className="absolute left-0 top-full mt-1 w-max bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg p-3 z-50 hidden group-hover:block max-w-[200px]">
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {displayNotes.map((note, index) => (
                    ...
                    >
                      <div className="text-xs text-gray-800 dark:text-slate-200 mb-1 line-clamp-2">
                        {note.note}
                      </div>
            ...
            cell: (row) => (
              <div className="relative">
                ...
                >
                  ...
                  {row.transaction_status !== "paid" && canEditLead && (
                    <DropdownItem
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-0.5 dark:text-slate-400">
            Manage and track your leads ({totalRows} total)
          </p>
        </div>
        ...
        <div className="flex gap-3">
              ...
              <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs size-5 rounded-full flex items-center justify-center">
                {
                  [
                    ...
                    selectedMerchant,
                    selectedPaymentStatus,
                    selectedSortBy?.value !== "created_at" && selectedSortBy,
          ...
          {canCreateLead && (
            ...
            >
              <BsPlus size={20} /> Add Lead
            </button>
          )}
        </div>
      </div>

      {/* Tabs - Only show if user can view test leads */}
      {canViewTestLeads ? (
