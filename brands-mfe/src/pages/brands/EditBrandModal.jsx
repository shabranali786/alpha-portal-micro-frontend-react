...
export default function EditBrandModal({
  ...
  useEffect(() => {
    if (brandDetails || brand) {
      const currentBrand = brandDetails || brand;

      const brandUnit = currentBrand.unit
        ? { value: currentBrand.unit.id, label: currentBrand.unit.title }
        : null;

      const brandMerchant = currentBrand.website_merchant
        ? {
            value: currentBrand.website_merchant.id,
            label: currentBrand.website_merchant.title,
          }
        : null;

      const brandEmailSetting = emailOptions.find(
        (option) =>
          option.value === (currentBrand.send_lead_to_email || "allow")
      );
      ...
      setFormData({
        ...
        unit_id: brandUnit,
        merchant_id: brandMerchant,
        invoice_link: currentBrand.invoice_link || "",
        detail_form_link: currentBrand.detail_form_link || "",
        send_lead_to_email: brandEmailSetting,
        alpha_chat_id: brandChat,

        logo: currentBrand.mail_details?.logo || "",
        name: currentBrand.mail_details?.name || currentBrand.title || "",
        email: currentBrand.mail_details?.email || "",
        phone: currentBrand.mail_details?.phone || "",
        url: currentBrand.mail_details?.url || currentBrand.domain || "",
        from_mail: currentBrand.mail_details?.from_mail || "",
        color_code: currentBrand.mail_details?.color_code || "#3B82F6",

        transport: currentBrand.mail?.transport || "",
        host: currentBrand.mail?.host || "",
        port: currentBrand.mail?.port || "",
        encryption: currentBrand.mail?.encryption || "",
        username: currentBrand.mail?.username || "",
        password: currentBrand.mail?.password || "",
      });
    }
  }, [brandDetails, brand]);
              ...
              <DialogPanel className="transform transition-all w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-2xl border border-transparent bg-white text-gray-900 dark:text-slate-100 shadow-xl dark:border-slate-700/60 dark:bg-slate-900 dark:shadow-black/40">
                ...
                <div className="border-b border-gray-200 dark:border-slate-700/60 px-6 py-4">
                  ...
                  <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">
                    Update brand information and configuration for:{" "}
