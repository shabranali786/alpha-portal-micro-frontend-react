...
const ViewExternalPaymentPage = () => {
  ...
  useEffect(() => {
    if (!canViewExternalPayment) {
      toast.error("You don't have permission to view external payment details");
      navigate("/external-payments");
    }
  }, [canViewExternalPayment, navigate]);
    ...
    const fetchInvoice = async () => {
      ...
      try {
        const response = await apiAxios.get(
          ApiRequest.externalPayments.show(id)
        );
        // console.log("show invoice", response.data);
        if (response.data.data) {
          setInvoice(response.data.data);
        } else {
          toast.error("External payment not found");
          navigate("/external-payments");
        }
      } catch (error) {
        console.error("Error fetching external payment:", error);
        toast.error(
          error.response?.data?.message || "Failed to load external payment"
        );
        navigate("/external-payments");
      } finally {
        setLoading(false);
...
export default ViewExternalPaymentPage;
