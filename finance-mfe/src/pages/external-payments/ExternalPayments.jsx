...
const ExternalPayments = () => {
  ...
  const canViewExternalPayment = usePermission(["external-payment.show"]);
  const canEditExternalPayment = usePermission(["external-payment.edit"]);
  const canDeleteExternalPayment = usePermission(["external-payment.delete"]);

  const canViewExternalPayments = usePermission(["external-payment.index"]);
  const canCreateExternalPayment = usePermission(["external-payment.create"]);
  ...
  const {
    ...
    queryParams,
    setQueryParams,
  } = usePaginatedData(ApiRequest.externalPayments.list);
  ...
  useEffect(() => {
    const savedColumns = localStorage.getItem("externalPaymentsTableColumns");
    if (savedColumns) {
  ...
  const handleBulkDelete = () => {
    if (selectedRows.length === 0) {
      toast.error("Please select external payments to delete");
      return;
    }
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      toast.success(
        `Successfully deleted ${invoiceIds.length} external payment(s)`
      );
      setActiveModal(null);
      setSelectedRows([]);
      setToggleCleared(!toggleCleared);
      ...
      console.error("Error deleting external payments:", error);
      toast.error(
        error.response?.data?.message || "Failed to delete external payments"
      );
    } finally {
  ...
  const hasAnyActionPermission =
    canViewExternalPayment ||
    canEditExternalPayment ||
    canDeleteExternalPayment;
  ...
  const handleAction = (action, row) => {
    ...
    switch (action) {
      case "view":
        navigate(`/external-payments/${row.id}`);
        break;
      case "view-invoice":
        window.open(
      ...
      case "print":
        ...
        break;
      case "edit":
        navigate(`/external-payments/${row.id}/edit`);
        break;
      case "delete":
  ...
  const handleDownloadInvoice = (invoice) => {
    toast.success(`Downloading external payment ${invoice.invoice_no}...`);
  };

  const handlePrintInvoice = (invoice) => {
    navigate(`/external-payments/${invoice.id}`);
    setTimeout(() => {
      window.print();
    }, 500);
  };
  ...
  const confirmDeleteInvoice = async () => {
    ...
    try {
      await apiAxios.delete(
        ApiRequest.externalPayments.delete(invoiceToDelete.id)
      );
      toast.success("External payment deleted successfully");
      setActiveModal(null);
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to delete external payment";
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold">External Payments Management</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-0.5">
            Manage and track all external payments
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <SearchBox
            ...
            placeholder="Search External Payments"
            size="md"
            icon="search"
          ...
          {canCreateExternalPayment && (
            <button
              onClick={() => navigate("/external-payments/create")}
        ...
        <div className="flex">
          ...
          >
            <div className="flex items-center justify-center gap-2">
              ...
              <span>Live External Payments</span>
              <span
          ...
          >
            <div className="flex items-center justify-center gap-2">
              ...
              <span>Test External Payments</span>
              <span
                className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                  activeTab === "test"
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-semibold text-gray-900 dark:text-slate-100">
                  {selectedRows.length} test external payment(s) selected
                </p>
          ...
          noDataComponent={
            <div className="w-full py-12 text-center dark:bg-slate-800">
              ...
              <div className="text-gray-500 dark:text-slate-300">
                {activeTab === "live" && "No live external payments found"}
                {activeTab === "test" && "No test external payments found"}
              </div>
                  ...
                  <div className="mb-6">
                    <p className="text-gray-700 dark:text-slate-300 mb-4">
                      ...
                      <span className="font-bold text-red-600 dark:text-red-400">
                        {selectedRows.length} test external payment(s)
                      </span>
                      .
                    </p>
                    ...
                    <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg dark:bg-red-900/10 dark:border-red-500/20">
                      <p className="text-sm text-red-800 dark:text-red-200">
                        ...
                        delete all selected test external payments and their
                        associated data.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      {deletingBulk ? (
                        ...
                        <>
                          <BsTrash size={16} />
                          Delete {selectedRows.length} External Payment(s)
                        </>
                      )}
                    </button>
                  </div>
      ...
      {/* Single External Payment Delete Modal */}
      <DeleteExternalPaymentModal
